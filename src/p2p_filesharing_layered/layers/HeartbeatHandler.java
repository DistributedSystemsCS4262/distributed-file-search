...
public class HeartbeatHandler extends Observable{
   ...
   // private final List<Neighbour> neighbours;
    ...
    private ArrayList<Neighbour> neighboursList;
    private ConcurrentMap<Integer, Set<Neighbour>> successors;
    ...
    HeartbeatHandler(Messenger messenger,Controller controller) {
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    isAlone = true;
                    neighboursList = controller.getNeighbours();
                    successors=controller.getSuccessors();
                    checkNeighbours();
    ...
    private Neighbour[] getSuccessors(Neighbour node) {
        Neighbour[] successors2 = new Neighbour[NO_OF_SUCCESSORS_TO_CHECK];
        
        Set<Neighbour> set = successors.get(node.hashCode());
        int i = 0;
        if (set != null) {
            for (Neighbour n : set) {
        ...
        for (Map.Entry<Neighbour, Boolean> entry : aliveNodes.entrySet()) {
            ...
            if (!alive) {
                ...
                if (temp == null) {//remove node  if there aren't any successors
                   // removeDeadNode(node);
                   controller.removeDeadNeighbour(node);
                    continue;
                }
    ...
    public void handleIsAliveResponse(AliveMessage response) {
        ...
        if (aliveNodes != null && aliveNodes.containsKey(succesorNode)) {
            ...
            if (deadNodeSuccesors.containsKey(succesorNode)) {
                ...
                if (controller.removeDeadNeighbour(deadNode)) {//remove deadNode form neighbour list
